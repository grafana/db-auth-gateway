...
// Provenance-includes-location: https://github.com/grafana/mimir/blob/main/pkg/util/usage/usage.go
...
import (
	...
	"github.com/grafana/db-auth-gateway/pkg/util/configdoc"
)

// Usage prints command-line usage for every registered flag.
// configs are expected to be pointers to structs; they are only used to look
// up per-field `doc` tags (e.g. hidden) and documented default overrides.
func Usage(configs ...interface{}) error {
	...
	fs.VisitAll(func(fl *flag.Flag) {
		...
		if v := reflect.ValueOf(fl.Value); v.Kind() == reflect.Pointer {
			field = fields[v.Pointer()]
		}
		if isFieldHidden(field, fl.Name) {
...
// Descending into some structs breaks default-value detection for some fields (eg. flagext.Secret),
// because the field itself is at the same memory address as the internal field in the struct.
